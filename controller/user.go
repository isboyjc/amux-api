...
const maxBatchManageUsers = 1000
...
// BatchManageUsers performs enable, disable, or soft-delete for a validated
// set of users in one database statement. All users are validated before any
// mutation so permission failures cannot leave a partially updated batch.
func BatchManageUsers(c *gin.Context) {
	...
	if len(req.IDs) == 0 || len(req.IDs) > maxBatchManageUsers {
		common.ApiErrorI18n(c, i18n.MsgInvalidParams)
		return
	}
	...
	if req.Action == "delete" {
		if err := model.DB.Where("id IN ?", uniqueIDs).Delete(&model.User{}).Error; err != nil {
			common.ApiError(c, err)
			return
		}
	} else {
	...
	for _, user := range users {
		if err := model.InvalidateUserCache(user.Id); err != nil {
			common.SysLog(fmt.Sprintf("failed to invalidate user cache after batch %s for user %d: %s", req.Action, user.Id, err.Error()))
		}
		if err := model.InvalidateUserTokensCache(user.Id); err != nil {
			common.SysLog(fmt.Sprintf("failed to invalidate tokens cache after batch %s for user %d: %s", req.Action, user.Id, err.Error()))
		}
		...
		if req.Action == "delete" {
			if err := events.PublishNoTx(events.UserDeleted, user.Id, &events.UserDeletedPayload{
				...
				common.SysError(fmt.Sprintf("publish user.deleted (admin batch soft) failed: %v", err))
			}
		}
	}
