...
// RecordQuotaLogWithAdminInfo 同时写入结构化 quota delta 与 Other.admin_info，
// 用于管理员手动调整用户额度：既保留按金额聚合的运营统计，又避免在用户可见的
// 日志内容中泄露操作管理员身份。
func RecordQuotaLogWithAdminInfo(userId int, logType int, quotaDelta int, content string, adminInfo map[string]interface{}) {
	if logType == LogTypeConsume && !common.LogConsumeEnabled {
		return
	}
	username, _ := GetUsernameById(userId, false)
...
func GetAllLogs(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, group string, requestId string, userId int, upstreamRequestId string) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
