...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	RecordTopupLog(topUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%.2f", logger.FormatQuota(int(quota)), topUp.Money), callerIp, topUp.PaymentMethod, "stripe")
...
func SearchAllTopUps(keyword string, status string, pageInfo *common.PageInfo) (topups []*TopupListItem, total int64, err error) {
	...
	if keyword != "" {
		...
		baseQuery = baseQuery.Where(
			"top_ups.trade_no LIKE ? ESCAPE '!' OR users.username LIKE ? ESCAPE '!' OR users.email LIKE ? ESCAPE '!'",
	...
	if err = baseQuery.Limit(searchTopUpCountHardLimit).Count(&total).Error; err != nil {
		tx.Rollback()
		common.SysError("failed to count search topups: " + err.Error())
		return nil, 0, errors.New("搜索充值记录失败")
	}
