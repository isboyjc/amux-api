...
func InitOptionMap() {
	...
	common.OptionMap["SeedanceWebhookEnabled"] = strconv.FormatBool(system_setting.SeedanceWebhookEnabled)
	common.OptionMap["SeedanceWebhookSecret"] = system_setting.SeedanceWebhookSecret
	...
	common.OptionMap["StripeUseDynamicPrice"] = strconv.FormatBool(setting.StripeUseDynamicPrice)
	common.OptionMap["StripeDisableAdaptivePricing"] = strconv.FormatBool(setting.StripeDisableAdaptivePricing)
	common.OptionMap["StripeProductName"] = setting.StripeProductName
	...
	common.OptionMap["AffShowInvitees"] = strconv.FormatBool(common.AffShowInvitees)
...
func updateOptionMap(key string, value string) (err error) {
	...
	case "EmailDomainBlacklist":
		if value == "" {
			common.EmailDomainBlacklist = []string{}
		} else {
	...
	case "SeedanceWebhookEnabled":
		system_setting.SeedanceWebhookEnabled = (value == "true")
	case "SeedanceWebhookSecret":
		system_setting.SeedanceWebhookSecret = value
	case "WorkerUrl":
	...
	case "StripeCurrency":
		if value != "USD" && value != "CNY" {
			return errors.New("StripeCurrency 必须是 USD 或 CNY")
		}
		setting.StripeCurrency = value
	case "StripeUseDynamicPrice":
		setting.StripeUseDynamicPrice = value == "true"
	case "StripeDisableAdaptivePricing":
		setting.StripeDisableAdaptivePricing = value == "true"
	case "StripeProductName":
		if value != "" {
			setting.StripeProductName = value
		}
	case "CreemApiKey":
	...
	case "AffRebateRatio":
		ratio, _ := strconv.ParseFloat(value, 64)
		if ratio < 0 {
			ratio = 0
		} else if ratio > 100 {
			ratio = 100
		}
		common.AffRebateRatio = ratio
	case "AffShowInvitees":
		common.AffShowInvitees, _ = strconv.ParseBool(value)
